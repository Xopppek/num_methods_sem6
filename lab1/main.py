...
num_line_h2, =  x_sol_axis.plot([], [], label = f'уголок h={h2}', color='dodgerblue', ls='--')
...
def update(t):
    ...
    err_temp = np.max(np.abs(analyt_sol(x_h1, t+1*tau(h1)) - h1_temp))
    ...
    err_temp = np.max(np.abs(analyt_sol(x_h2, t + 10 * tau(h2)) - h2_temp))
    ...
    x_err_axis.set_title(r"""$\epsilon_1/\epsilon_2$ =""" + f'{err_h1.get_ydata()[-1]/(err_h2.get_ydata()[-1] + 0.0000000001):.2f}')
